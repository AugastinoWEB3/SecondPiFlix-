...
export const MobileBottomNav: React.FC = () => {
  ...
  const navItems = [
    ...
    { id: 'movies', label: t('movies', 'Movies'), icon: <Film className="w-5 h-5" /> },
    { id: 'series', label: t('series', 'Series'), icon: <Tv className="w-5 h-5" /> },
    { id: 'search', label: t('search', 'Search'), icon: <Search className="w-5 h-5" /> },
    { id: 'watchlist', label: t('myList', 'My List'), icon: <Bookmark className="w-5 h-5" /> },
    { id: 'premium', label: 'VIP', icon: <Sparkles className="w-5 h-5 text-amber-400" /> }
  ];

  return (
    <div id="mobile-bottom-nav" className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-[#0c0d14]/95 backdrop-blur-xl border-t border-zinc-800/80 px-2 py-2 safe-area-pb">
